...
function Dashboard({ onLogout, userName, userId }) {
  ...
  const handlePackageChange = async (e) => {
    ...
    setShowUnavailableMsg(value === 'cooperative');

    try {
      await updateDoc(doc(db, 'users', userId), { selectedPackage: value });
    } catch (error) {
  ...
  const handleProceed = () => {
    if (step === 1 && selectedPackage !== '' && selectedPackage !== 'cooperative') {
      setStep(2);
    } else if (step === 2 && contributionAmount) {
      setStep(3);
    }
  };
    ...
    <div className={styles.dashboardContainer}>
      ...
      {step === 1 && (
        <div>
          ...
          <button onClick={handleProceed} disabled={!selectedPackage || selectedPackage === 'cooperative'}>Proceed</button>
        </div>
      )}
      ...
      {step === 2 && (
        <div>
          ...
          <input type="number" id="amount" value={contributionAmount} onChange={handleAmountChange} placeholder="Enter amount" />
